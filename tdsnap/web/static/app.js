...
function addWords(raw) {
  ...
  raw
    ...
    .filter(Boolean)
    .forEach((word) => {
      if (state.words.length < capacity && !state.words.includes(word)) {
...
function renderPreview() {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    if (index < state.words.length) {
      ...
      cell.textContent = state.words[index];
    }
    preview.append(cell);
  }
}
...
$("ai-go").addEventListener("click", async () => {
  ...
  setBusy(button, true);
  status.textContent = `Asking ${$("ai-model").value} for ${$("ai-count").value} “${category}” words…`;
  try {
    ...
    addWords(data.words.join(","));
    status.textContent = `Added ${data.words.length} suggestions — remove any you don't want.`;
  } catch (error) {
    status.classList.add("error");
    status.textContent = error.message;
  } finally {
