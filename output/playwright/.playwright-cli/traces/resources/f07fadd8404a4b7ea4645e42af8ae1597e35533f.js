...
const PREVIEW_ICONS = {
  ...
  positive: '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="m12 4 2.25 4.65 5.1.7-3.7 3.55.9 5.05L12 15.6l-4.55 2.35.9-5.05-3.7-3.55 5.1-.7L12 4Z" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/></svg>',
  negative: '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="8.5" stroke="currentColor" stroke-width="1.8"/><path d="M8.5 12h7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
...
function renderPreview() {
  ...
  for (let slot = 0; slot < total; slot += 1) {
    ...
    if (existing) {
      ...
      addPreviewCellContent(cell, existing.label || "Existing button");
    ...
    if (index >= 0) {
      ...
      addPreviewCellContent(cell, item.label, item.fn, item.symbol !== false);
      cell.draggable = true;
      cell.tabIndex = 0;
...
renderWords();
renderPreview();
show(state.profile.assessed ? "load" : "welcome");
